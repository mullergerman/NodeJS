...
class ApiProductosBO{
    ...
    async initialize(config){
        ...
        this.daoProductos = getDaoProductos(config);
        await DaoInit(this.daoProductos);
    }
    ...
    async getAll(){
        const result = await this.daoProductos.getAll();
        return result;
    }

    async get(num){
        const producto = await this.daoProductos.get(num)
        if (!producto){
    ...
    async insert(producto){
        ...
        const resultado = await this.daoProductos.insert(producto)                    
        if (!resultado){
    ...
    async update(id, productoDto){
        ...
        const producto = this.daoProductos.get(id)
        if (!producto){
        ...
        const resultado = await this.daoProductos.update(id, productoUpgrade);
        if (!resultado){
    ...
    async remove(id){
        ...
        const resultado = await this.daoProductos.remove(id);
        if (!resultado){
    ...
    async deduct(producto_id){
        // Obtener el producto
        let product = await this.daoProductos.get(producto_id);
        if (!product){
        ...
        if (product.stock == 0){
            ...
            resultado = await this.daoProductos.update(product.id, product);
            if (!resultado){
