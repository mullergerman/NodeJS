...
const daoCarritos = getDaoCarritos();
DaoInit(daoCarritos);
...
class ApiCarritosBO{
    ...
    async newCarrito(username){
        ...
        const result = await daoCarritos.insert(carrito);
    ...
    async removeCarrito(carrito_id, username){
        ...
        const carrito = daoCarritos.get(carrito_id);
        if (!carrito){
        ...
        const resultado = await daoCarritos.remove(carrito_id);
        if (!resultado){
    ...
    async getProductoInCarrito(carrito_id){
        const carrito = await daoCarritos.get(carrito_id);
        if (!carrito){
    ...
    async insertProductInCarrito(carrito_id,producto){
        ...
        const result = await daoCarritos.update(carrito_id,carrito)
