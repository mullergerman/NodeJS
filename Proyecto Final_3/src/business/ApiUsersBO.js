...
class ApiUsersBO{
    ...
    async register(userDTO){
        ...
        const users = await daoUsers.getByUsername(userDTO.username)
        ...
        userDTO.cart = "";
        const resultado = await daoUsers.insert(userDTO);
        if (!resultado){
    ...
    async deserialize(username){
        const user = await daoUsers.get(username);
        if (!user){
            return null
        }else{
            return user
    ...
    async addCartToUser(username, carrito_id){
        ...
        const result = await daoUsers.update(username, user);
        if (!result){
            logger.error(`ApiUsersBO;addCartToUser;Imposible Asignar Carrito`);
            return ({err: this.error.ImposibleAsignarCarrito, user: null})
        }
    ...
    async setUserCart(username, carrito_id){
        let user = daoUsers.get(username);
        user.cart = carrito_id;
        const result = await daoUsers.update(username, user);
        if (!result){
    ...
    async removeUserCart(username){
        let user = await daoUsers.get(username);
        user.cart = "";
        const result = await daoUsers.update(username, user);
        if (!result){
